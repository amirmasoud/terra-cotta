  ...
  <div>
    <div class="bg-white p-6 rounded-lg shadow-lg">
      <div class="flex items-center">
        <button @click="$emit('open')"><edit-svg class="p-1" /></button>
        <span
      ...
      <div class="flex flex-wrap">
        <div v-for="(field, index) in safe.fields" :key="index" class="w-full mt-1">
          ...
          <button v-if="field.type == 'password'" class="hover:bg-gray-300 rounded-full float-left">
            <eye-svg class="p-1" />
          </button>
        </div>
      </div>
